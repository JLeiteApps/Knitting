...
2. **Add pattern → parse review** — staged pipeline over the pasted/uploaded
   text: "Pattern units" dropdown (in/cm — DECLARED, never detected) → pdf.js
   client-side text (scanned pages flagged) → notation layer (gauge incl. metric
...
3. **Fit profile** — Herzog fields with measurement guidance; "Show measurements
   ...
   profile's unit drives the whole UI.
...
- Intent drafting is heuristic until `LLM_ENDPOINT`/`LLM_API_KEY` exist.
